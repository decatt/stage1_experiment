...
class Agent(nn.Module):
    def __init__(self, frames=4):
        ...
        self.network = nn.Sequential(
            ...
            nn.ReLU(), )
        self.actor = layer_init(nn.Linear(256, action_space.sum()), std=0.01)
    ...
    def get_action(self, x, action=None, invalid_action_masks=None, envs=None):
        ...
        if action is None:
            ...
            split_invalid_action_masks = torch.split(invalid_action_masks, action_space.tolist(), dim=1)
...
invalid_action_masks = torch.zeros((args.num_steps, args.num_envs) + (action_space.sum(),)).to(device)
