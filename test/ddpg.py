...
class DDPG(object):
    ...
    def learn(self):
        ...
        bs = torch.FloatTensor(np.array([d[0] for d in data]).reshape((batch_size, 16, 16, 27))).to(device)
        ba = torch.FloatTensor(np.array([d[1] for d in data])).to(device)
        br = torch.FloatTensor(np.array([d[2] for d in data])).to(device)
        bs_ = torch.FloatTensor(np.array([d[3] for d in data]).reshape((batch_size, 16, 16, 27))).to(device)

        a = self.Actor_eval(bs)
        q = self.Critic_eval(bs, a)  # loss=-q=-ce（s,ae（s））更新ae   ae（s）=a   ae（s_）=a_
        # 如果 a是一个正确的行为的话，那么它的Q应该更贴近0
        loss_a = -torch.mean(q).to(device)
        # print(q)
        # print(loss_a)
        ...
        # print(q_v)
        td_error = self.loss_td(q_target, q_v).to(device)
